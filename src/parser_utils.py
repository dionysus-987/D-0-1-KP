...
def parse_dkp_instances(file_path: str) -> Dict[str, DKPInstance]:
    ...
    # 统一匹配四类实例名：IDKP1:, SDKP2:, UDKP3:, WDKP4:
    # 使用捕获组是为了 split 后保留实例名
    pattern = re.compile(r'((?:I|S|U|W)DKP\d+\s*:)', re.IGNORECASE)
    parts = pattern.split(raw)

    if len(parts) < 3:
        raise ValueError("未识别到任何 DKP 实例，请检查 txt 文件格式。")
    ...
    for i in range(1, len(parts), 2):
        ...
        if not profit_match:
            profit_match = re.search(
                r'The\s+profit\s+of\s+(?:itmes|items)\s+are\s*:\s*(.*?)\s*The\s+weight\s+of\s+(?:itmes|items)\s+are\s*:',
                block,
                re.IGNORECASE | re.DOTALL
            )
        if not profit_match:
        ...
        if len(profits) < dimension:
            raise ValueError(
                f"{name}: profit 数量不足，期望 {dimension} 个，实际 {len(profits)} 个。"
            )

        # weight 段：取当前 block 内后续所有数字，再截取前 dimension 个
        ...
        if not weight_match:
            weight_match = re.search(
                r'The\s+weight\s+of\s+(?:itmes|items)\s+are\s*:\s*(.*)',
                block,
                re.IGNORECASE | re.DOTALL
            )
        if not weight_match:
        ...
        if len(weights) < dimension:
            raise ValueError(
                f"{name}: weight 数量不足，期望 {dimension} 个，实际 {len(weights)} 个。"
            )
